"""Telegram-бот: дневник калорий с быстрым вводом и уведомлениями о достижении целей.
...
def db_init():
    ...
    # Таблица уведомлений — какие плашки уже показывали сегодня
...
def db_notif_seen(uid, kind):
    """Возвращает True, если уведомление уже показывали сегодня."""
    con = sqlite3.connect(DB_PATH)
    r = con.execute(
...
def db_notif_mark(uid, kind):
    """Помечает уведомление как показанное сегодня."""
    con = sqlite3.connect(DB_PATH)
    con.execute(
...
def check_achievements(uid, user, sums):
    """Возвращает список новых уведомлений, которые надо показать."""
    notifications = []
    sum_k, sum_p, sum_f, sum_c = sums

    # Калории — достижение нормы (90-110% от цели)
    ...
    # Калории — серьёзное превышение (на 200+ ккал больше нормы)
...
async def add_meal(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Проверяем достижения и отправляем уведомления (каждое — отдельным сообщением)
    achievements = check_achievements(uid, user, (sum_k, sum_p, sum_f, sum_c))
